"""
Run have log file:( mkdir logs 2>nul
start "RUN_CHUNKING" cmd /k "python -u main.py > logs\run_chunk_15012026_v0_p2.log 2>&1" )
Kill: (tasklist /v | findstr RUN_CHUNKING)( taskkill /IM python.exe /F ) -> (taskkill /F /PID 25110)
...
def _process_one_pdf_to_txt(
    ...
    try:
        # ✅ sanity check paths (rất hay fail do relative path)
        ...
        # ✅ trả traceback đầy đủ để debug nhanh
...
def pdfs_to_mirrored_txt(
    ...
    *,
    ...
    # ✅ convert relative -> absolute ONCE (cực quan trọng khi dùng ProcessPool)
    ...
    def _is_real_pdf(p: Path) -> bool:
        ...
        if p.suffix.lower() != ".pdf":
            return False
        # ✅ bỏ file lock/tạm của Office/Windows
        if name.startswith("~$"):
            return False
        # ✅ optional: bỏ mấy pdf siêu nhỏ (thường là rác / file lỗi)
        try:
    ...
    if not pdf_paths:
        ...
        (output_txt_root / "_stats.json").write_text(json.dumps(stats, ensure_ascii=False, indent=2), encoding="utf-8")
        return stats

    # ✅ Nếu dùng GPU, đừng chạy nhiều process (2 process xài 1 GPU rất hay crash / fail)
    if clip_device and str(clip_device).lower().startswith("cuda"):
        max_workers = 1
    else:
        max_workers = max(1, int(max_workers))

    pbar = tqdm(total=len(pdf_paths), desc="1/4 PDFs -> TXT (parallel)", unit="pdf")
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as ex:
        for pdf_path in pdf_paths:
            fut = ex.submit(
                ...
                str(output_txt_root),
                str(work_root),
                prototypes_path,        # ✅ already absolute
                labels_heading_json,    # ✅ already absolute
                n_pages,
                test_fast,
    ...
    (output_txt_root / "_timings.json").write_text(
        json.dumps(sorted(timings, key=lambda x: x["seconds"], reverse=True), ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
...
if __name__ == "__main__":
    paths = run_e2e_pdf_folder_to_chunked_json(
        input_pdfs_root=r"E:\AIBuddy\dataset\run",
        out_root=r".\outputs\ttnt_new",
        ...
        label_mode="artifacts",
        recursive=True,
        overwrite_txt=False,
        cleanup_work=False,
        pdf_max_workers=2,  # (GPU sẽ bị ép xuống 1 ở trong pdfs_to_mirrored_txt)
        min_tokens=250,
        target_tokens=400,
        max_tokens=650,
        overlap_units=2,
