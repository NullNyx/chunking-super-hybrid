"""
B1 - PDF Text Extraction / Trích xuất text từ PDF

Input:
- PDF file

Output:
- Markdown text with heading markers (## Heading:)

Workflow:
1. Parse PDF using Docling (document converter)
2. Use OCR fallback via olmOCR if needed (garbled fonts)
3. Detect headings using CLIP model (image classification)
4. Normalize markdown (fix Vietnamese glyphs, whitespace)
"""
...
def _get_docling_converter(
    images_scale: float = 1.2, generate_picture_images: bool = True
):
...
def load_prototypes(
    ...
    """
    protos: Dict[str, torch.Tensor] = torch.load(
        str(prototypes_path), map_location="cpu"
    )
    ...
    *,
    ...
    for i in range(0, len(pil_images), BATCH_SIZE):
        ...
        inputs = {
            k: (
                v.to(device).half()
                if v.is_floating_point() and "cuda" in device
                else v.to(device)
            )
        ...
        with torch.inference_mode():
            ...
            feats = (
                F.normalize(feats, dim=1).float().cpu()
            )  # [B, D] — cast back to float32 for matmul with prototypes
        ...
        for j, (path, sim_row) in enumerate(zip(batch_paths, sims)):
            ...
            if score >= score_threshold:
                hits.append(
                    ClipHit(image_path=str(path), label=labels[best_idx], score=score)
                )
...
def inject_headings(
    *,
    ...
    def _inject_text_headings(txt: str) -> Tuple[str, int]:
        ...
        # --- Pass 1: Parse TOC ---
        _TOC_RE = re.compile(r"\|\s*\|\s*Bài\s+(\d+)\.\s*(.+?)\s*\|\s*(\d+)\s*\|")
            ...
            for line in lines:
                ...
                if m2 and len(stripped) < 120:
                    ...
                    if ln not in seen_lessons:
                        seen_lessons.add(ln)
                        if out_lines and not out_lines[-1].strip().startswith(
                            "## Heading:"
                        ):
                ...
                if (
                    ...
                    if matched_idx >= 0:
                        ...
                        if skipped_titles:
                            ...
                            step = (
                                gap // (len(skipped_titles) + 1)
                                if gap > len(skipped_titles)
                                else 1
                            )

                            # Insert in reverse to preserve indices
                            for si, (sk_lesson, sk_title) in enumerate(
                                reversed(skipped_titles)
                            ):
                                insert_pos = last_heading_pos + step * (
                                    len(skipped_titles) - si
                                )
                                insert_pos = min(insert_pos, current_pos)
                                out_lines.insert(
                                    insert_pos,
                                    f"## Heading: Bài {sk_lesson}. {sk_title}",
                                )
                                count += 1
        ...
        if title_queue:
            ...
            for insert_idx, heading_text in sorted(
                inserts, key=lambda x: x[0], reverse=True
            ):
                out_lines.insert(insert_idx + 1, heading_text)

            print(
                f"  [INFO] {len(remaining)} lessons injected by position estimate: {', '.join(f'Bài {n}' for n, _ in remaining)}",
                flush=True,
            )
...
def collect_images_for_clip(
    *,
    ...
    return [
        p for p in artifacts_dir.iterdir() if p.is_file() and p.suffix.lower() in exts
    ]
...
def run_one_pdf(
    ...
    *,
    ...
    (out_dir / "images.json").write_text(
        json.dumps(ex.images, ensure_ascii=False, indent=2), encoding="utf-8"
    )

    # (2) CLIP labeling
    image_paths = collect_images_for_clip(
        ex=ex, pdf_path=pdf_path, label_mode=label_mode
    )
