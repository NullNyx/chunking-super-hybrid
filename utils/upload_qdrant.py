"""
Upload to Qdrant / Upload JSON chunks lên Qdrant vector DB

Input:
- JSON chunks từ B4 (merged per-book JSONs)

Output:
- Embeddings upserted vào Qdrant collection

Workflow:
1. Load JSON chunks
2. Embed sử dụng dense ( HuggingFace) + sparse (BM25-like)
3. Upsert points vào Qdrant với retry logic
4. Parallel upload với multiprocessing

Dependencies:
- fastembed (sparse embeddings)
- langchain_huggingface (dense embeddings)
- qdrant_client
